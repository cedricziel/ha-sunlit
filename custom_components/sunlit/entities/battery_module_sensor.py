...
class SunlitBatteryModuleSensor(CoordinatorEntity, SensorEntity):
    ...
    async def async_added_to_hass(self) -> None:
        ...
        The primary coordinator (passed to ``CoordinatorEntity``) is the device
        coordinator. The MPPT energy values come from a separate MPPT
        coordinator, which HomeAssistant only schedules for periodic refresh
        while it has at least one listener. Without this subscription the MPPT
        coordinator would refresh once at startup (all energy = 0) and never
        again, freezing MPPT Total Energy at 0 (issue #72).
        """
